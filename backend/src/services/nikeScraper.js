...
exports.scrapeProductData = async (url) => {
  ...
  try {
    ...
    // Verificar se estamos em uma página de produto da Nike
    const isNikeProductPage = await page.evaluate(() => {
      return (window.location.href.includes('nike.com.br/') || window.location.href.includes('nike.com/br/')) && 
             (document.querySelector('.product-info-container') !== null ||
              document.querySelector('.product-card') !== null ||
              document.querySelector('[data-test="product-title"]') !== null);
    });
    
    if (!isNikeProductPage) {
      console.log('Não estamos em uma página de produto da Nike. Procurando links...');
      
      // Tentar encontrar link de produto
      const productLink = await page.evaluate(() => {
        // Buscar links que apontam para produtos da Nike
        const links = Array.from(document.querySelectorAll('a')).filter(a => 
          a.href && (a.href.includes('nike.com.br/') || a.href.includes('nike.com/br/')) && 
        ...
        if (links.length > 0) {
          ...
          const productLinks = links.filter(a => 
            a.href.includes('/produto/') || a.href.includes('/product/')
          );
    ...
    const productData = await page.evaluate(() => {
      ...
      const productTitle = 
        document.querySelector('h1.title-product')?.textContent.trim() || 
      ...
      // Imagem do produto
      const productImage = 
        document.querySelector('.photo-product-zoom img')?.src ||
        document.querySelector('.product-image img')?.src ||
        document.querySelector('[data-test="product-image"] img')?.src ||
    ...
    throw new Error(`Falha ao extrair dados do produto na Nike: ${error.message}`);
  } finally {
    await browser.close();
  }
};